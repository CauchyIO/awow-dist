...
`../../context/tooling/knowledge-sources.md`, for the record profile and full contract.
...
2. With no credible match, remain HUB-only. With one match, use it. With several plausible
...
External sources are read-only. Never write back to a spoke, SharePoint, vector store, or other
canonical system while acting from the HUB.

## Capture

Before any durable HUB write, decide where the fact is canonical. Write HUB-canonical knowledge
...
- Missing or empty catalog: proceed HUB-only.
- Missing access capability: report the source, URI, and missing capability; continue with HUB
...
- Any access path that would require copying content into the HUB or writing externally: stop.
