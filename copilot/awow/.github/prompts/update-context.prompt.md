...
[2] UNROUTED — no existing file covers release timing
    ...
    Staged in the knowledge inbox as a guidance candidate. awow-workflows' /kb-synthesize drains it.

Dropped: 1
  - "stop saying you're absolutely right" — P3: one person's preference, not a team rule.

Reply with one of: `1` or `all` — apply · `none` — apply nothing · `none, stop asking` — and never offer again · `2 → <path>` — put #2 in that file instead · `1 defer` — stage #1 in the inbox
```
