...
phase: kickoff
argument-hint: "[<board-url>] [--anchor <git-url>] [--check]"
prerequisites: []
removes_pain: "the I-installed-the-plugin-now-what problem"
channel: bootstrap
---
...
2. Report three things, each as `✓ ok`, `⧗ incomplete` or `✗ broken` — always the word after the symbol — with the reason and the fix in one line:
   - **Context** — which repo's awow context applies here (the `board-target` skill settles it; say it as "this repo" or "the anchor at <url>", never "installation"), whether `context/tooling/board.md` exists, and which of the conventions and the team profile are still absent. An absent convention or profile is `⧗`, never `✗`: the command that first needs it offers to draft it.
   - **Anchor** — whether this repo is anchored (root `AGENTS.md` frontmatter), and if so whether the anchor resolves: `$AWOW_ANCHOR`, else `.awow/anchor.json`, and whether the recorded checkout's `origin` still matches the committed URL. Standalone: say so; that is `✓`, not a gap.
...
3. Close with one line: what plain `/setup-awow` would propose to repair, or `nothing to repair`. Then stop — no situation, no diff, no offer to continue.
...
Present the diff as one list, one line per file: the path; whether it is new, changed, local-only or in the anchor; its effect in a few words; and for a convention its tag (`observed — proposal` / `default — proposal`). Tags and "proposal" wording belong to the diff, not to any landed file. Print any file's exact content on request. Then ask once — "Land these? Reply `go` to land all, `strike 2, 5` to drop lines, `show 3` for a file's exact content, or `cancel`." — and accept a partial answer or any plain yes.
