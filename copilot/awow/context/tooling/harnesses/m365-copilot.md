...
Emits `dist/m365/awow/appPackage/` (manifest, declarative agent, plugin + OpenAPI specs, icons) from `.agents/` + `context/`, the same generated-payload contract as `dist/`. Never hand-edit files under `dist/m365/awow/` — they're overwritten on the next run. The plan sees every file git would commit, staged or not; only ignored files are left out.
...
```bash
cd dist/m365/awow/appPackage && zip -r ../awow-m365.zip .
```
