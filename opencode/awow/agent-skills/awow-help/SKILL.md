...
**The catalog is your only source of command names.** Read `../../context/tooling/command-catalog.md` first: every command awow ships, one line each, and the plugin it ships in. Never name a command that is not in it, and never describe a command in words other than its catalog line plus what you observed.
...
With `--commands`: one table for the `awow` commands and one for `awow-workflows` when it is installed, each row the command with its flags and its catalog line as written. When `awow-workflows` is absent, one line after the first table: what the bundle adds and the exact install command for this harness, quoted from the catalog's `Installing awow-workflows` table — never a guessed or hedged one. Mark a command whose prerequisite this repo plainly lacks; never hide it.
...
2. No `board.md` and no `anchor:` → `/setup-awow <board-url>`, or `/setup-awow --anchor <git-url>` when the team already keeps its awow context in a shared repo.
...
The next step is the answer; put it before anything else. Without `--commands`:
