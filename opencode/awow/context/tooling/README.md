...
The team's actual tooling choices live in `board.md` (written by `/setup-awow`). The reference files under `boards/<tool>/reference/` and `harnesses/` describe what each tool brings to the agentic operating model; the wizard reads them at setup time and writes the team's resolved choices into `board.md`.

## Files

- `board.md` — the team's actual board spec (single source of truth). Written by `/setup-awow` from the live board.
