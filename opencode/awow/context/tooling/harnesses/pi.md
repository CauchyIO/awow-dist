...
A `.pi/` directory (Pi's per-repo config), or the user explicitly chooses Pi. `/setup-awow` detects the `.pi/` signal.
...
- `.agents/commands/*` and `.agents/skills/*` → the commands-as-skills surface (`gather.py --surface plugin`, into `dist/pi/awow/agent-skills/`), registered through `pi.skills` in the `package.json` at the `dist/` root — Pi installs a whole repo as one package, so that manifest has to sit at the root and point into the harness folder.
