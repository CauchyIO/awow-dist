...
Through the awow Copilot plugin. `tools/gather.py` builds it into `dist/copilot/awow/` — the manifest from `.github/plugin/plugin.json`, every command and skill rendered as a Copilot agent skill, and the prompts under `.github/prompts/` with the `.prompt.md` suffix VS Code requires — and `copilot plugin marketplace add CauchyIO/awow` installs it, through `.github/plugin/marketplace.json` (Copilot CLI checks that location before `.claude-plugin/marketplace.json`, which serves Claude Code its own folder). Nothing is copied into a repo's own `.github/`: an adopter's `copilot-instructions.md` or `AGENTS.md` there is the team's own file.
