...
#
# The plugin's own root is tried FIRST, deliberately: an installed plugin must
# load its dependency from inside its own payload, never from a checkout it
# happens to sit in. (Before the dist/<harness>/<plugin>/ layout the payload sat
# at <clone>/dist-telemetry/, which made parents[4] the awow clone — a second,
# identical tools/mlflow_reader.py that the wrong order would have picked up
# unnoticed. The order stays, so the hazard cannot come back with a relayout.)
#
