...
> **One example backend.** This skill ships as a starter for teams whose agent sessions land in **Databricks MLflow**. Teams on a different tracing backend (LangSmith, Helicone, OpenTelemetry, raw session JSONL on disk, etc.) should swap this skill for their backend's equivalent as a repo-local skill. The downstream skills (`prompt-skill-analysis`, `awow-usage-coach`) consume the JSON layout this skill produces, so any replacement should match the output shape documented below.
...
> **The export is private session data — never commit it.** Traces contain verbatim prompts, real names, private issue IDs, infra details, and secrets users pasted. The default output dir `mlflow_export/` is gitignored; keep it that way. Do not move the export, or any report derived from it, into a tracked path (`{PROJECT}/proposals/`, `{ANCHOR}/context/`, …) — if the repo is public, that leaks customer data.
