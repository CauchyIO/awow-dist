...
> **Starter shape — adjust for your harness.** This skill ships supporting two input shapes: raw **Claude Code** session JSONL and the JSON layout produced by the bundled `mlflow-export` skill. Teams running primarily on GitHub Copilot, Cursor, or another harness should extend `scripts/extract_prompts.py` with a reader for their session format as a repo-local skill. The report rubric below is harness-agnostic — only the input parsing is tied to a specific shape.
