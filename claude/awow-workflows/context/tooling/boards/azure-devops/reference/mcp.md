...
2. Establish write access **without writing** — a no-op write still bumps the updated time, adds a history entry, and can notify the team. Use a permission read if the surface offers one; otherwise, with write tools present, record `write-unverified` and let the first approved real write settle it.
