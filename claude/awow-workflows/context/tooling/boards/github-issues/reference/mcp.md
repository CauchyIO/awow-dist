...
2. Establish write access **without writing** — a no-op write still bumps the updated time, adds a history entry, and can notify the team. Read the repository's `permissions` for the authenticated user through the MCP's repository lookup: `push` or `triage` proves Issues write (`write-verified`); neither is `write-denied`. No permissions in the response: `write-unverified`, settled by the first approved real write.
3. Record the verification status in `context/tooling/board.md`.

## Option 2 — `gh` CLI (lighter; reuse existing GitHub auth)

If the user already has `gh` installed and authenticated for the org/repo, the agent can shell out to it via the harness's Bash tool. This is what awow's own retired dogfood workspace used — see `tests/fixtures/fikkert/_seed/context/tooling/board.md` for a worked example.
...
3. `gh api repos/<owner>/<repo> --jq .permissions` — write access, proven without writing: `push` or `triage` true is `write-verified`, neither is `write-denied`. Never edit an issue to find out — a no-op edit still bumps its updated time and can notify the team.
4. Record `surface: gh-cli` in `context/tooling/board.md`.

### When `/setup-awow` picks this option
