...
once, then mine it for knowledge worth keeping in `{ANCHOR}/context/knowledge-base/`. This is
...
`{ANCHOR}/context/knowledge-base/mining.md`, falling back to
...
`{ANCHOR}/context/knowledge-base/`). `mining.md` honours the same config, so this command
...
- **Canonical sources, first run only.** When `{ANCHOR}/context/knowledge-sources/index.md`
  ...
  `{ANCHOR}/context/tooling/knowledge-sources.md` (else `${CLAUDE_PLUGIN_ROOT}/context/tooling/knowledge-sources.md`)
...
Run the shared collection step — read `{ANCHOR}/context/tooling/activity-collection.md`,
...
Follow `{ANCHOR}/context/knowledge-base/mining.md`, falling back to
...
each survivor to its `{ANCHOR}/context/knowledge-base/` destination, dedup against the existing
...
promote. Promotion follows the ritual in `{ANCHOR}/context/knowledge-base/README.md`,
...
- **Proposes only.** No write into `{ANCHOR}/context/knowledge-base/` and no board mutation
