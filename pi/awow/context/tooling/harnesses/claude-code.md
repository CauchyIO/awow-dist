...
Nothing is copied into a repo's `.claude/`: the team's own `CLAUDE.md` there is theirs, and awow never regenerates it; `/setup-awow` only appends a short pointer to the root `AGENTS.md`. A legacy vendored tree still mirrors `.agents/` into `.claude/` with its own `tools/gather.py`; that route is retired for new installs.

## Settings

`.claude/settings.json` is the harness configuration: permissions, MCP servers, environment variables, hooks, and the project-scope plugin enablement. `/setup-awow` prints the MCP snippet for the team's board for the user to add, and enables the awow plugin at project scope.
